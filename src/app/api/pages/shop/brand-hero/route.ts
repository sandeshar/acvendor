...
export async function POST(request: NextRequest) {
    try {
        ...
        const { brand_slug, badge_text, tagline, title, highlight_text, subtitle, description, card_overlay_text, card_cta_text, card_cta_link, cta_text, cta_link, cta2_text, cta2_link, background_image, hero_image_alt, is_active, display_order } = body;
...
export async function PUT(request: NextRequest) {
    try {
        await connectDB();
        const body = await request.json();
        const { id, brand_slug, badge_text, tagline, title, highlight_text, subtitle, description, card_overlay_text, card_cta_text, card_cta_link, cta_text, cta_link, cta2_text, cta2_link, background_image, hero_image_alt, is_active, display_order } = body;
        ...
        if (card_overlay_text !== undefined) updateData.card_overlay_text = card_overlay_text;
        if (card_cta_text !== undefined) updateData.card_cta_text = card_cta_text;
        if (card_cta_link !== undefined) updateData.card_cta_link = card_cta_link;
