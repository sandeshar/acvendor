        ...
        <main className="flex flex-1 flex-col items-center py-5 md:py-10">
            <div className="layout-content-container flex flex-col w-full max-w-7xl px-4 md:px-10 flex-1 gap-8">
                ...
                <div className="flex flex-wrap gap-2 px-0 text-sm md:text-base">
                    ...
                    <Link href={post.category?.slug ? `/products?category=${encodeURIComponent(post.category.slug)}` : '/products'} className="text-[#617589] font-medium leading-normal hover:text-primary">{post.category?.name || 'Residential'}</Link>
                    <span className="text-[#617589] font-medium leading-normal">/</span>
                    {post.subcategory?.slug ? (
                        <Link href={`/products?category=${encodeURIComponent(post.category?.slug || '')}&subcategory=${encodeURIComponent(post.subcategory.slug)}`} className="text-[#617589] font-medium leading-normal hover:text-primary">{post.subcategory?.name || 'Inverter Series'}</Link>
                    ) : (
