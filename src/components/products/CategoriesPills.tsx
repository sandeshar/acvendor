                ...
                return (
                    <div key={catKey} className="flex items-center gap-2">
                        ...
                        {subcats.filter(sc => String(sc.category_id) === catIdStr).slice(0, 3).map((sc: any, scIdx: number) => (
                            <Link key={sc._id ?? sc.id ?? sc.slug ?? scIdx} href={`/midea-ac?category=${encodeURIComponent(c.slug)}&subcategory=${encodeURIComponent(sc.slug)}`} className={`px-2 py-1 rounded-full text-sm ${selectedSubcategory === sc.slug ? 'bg-primary/10 text-primary font-medium' : 'bg-white text-[#617589] border border-gray-100'}`}>
                                {sc.name}
                            </Link>
                        ))}
                    </div>
