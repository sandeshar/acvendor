...
const AboutCertifications = ({ certifications = [], section }: { certifications?: CertificationItem[]; section?: CertificationsSection | null }) => {
    const items = (certifications && certifications.length > 0) ? certifications : [
        ...
        <section className="px-4 md:px-10 lg:px-40 flex flex-1 justify-center py-12 bg-background-light border-t border-[#e5e7eb]">
            <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
                <div className="flex flex-col gap-8 text-center">
                    <h3 className="text-[#111418] text-2xl font-bold">{section?.title || 'Trusted by Industry Leaders'}</h3>
                    <p className="text-sm text-[#617589] mt-2">{section?.subtitle || 'Authorized Sales & Service Partners for Major Global Brands'}</p>
